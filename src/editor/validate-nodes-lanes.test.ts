...
describe("serializeNodes / serializeLanes — round-trip through the loader", () => {
  ...
  it("both preserve the gen-yaml header so diffs stay clean", () => {
    expect(serializeNodes(META, NODES)).toMatch(/^# GENERATED by scripts\/gen-yaml\.ts/);
    expect(serializeLanes(LANES)).toMatch(/^# GENERATED by scripts\/gen-yaml\.ts/);
  });
});
